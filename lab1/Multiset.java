...
public class Multiset {
    ...
    public void createMultisetFromString(String str) {
        Stack<Multiset> stack = new Stack<>();

        StringBuilder currentElement = new StringBuilder();
        boolean isNestedMultiset = false;

        Multiset currentMultiset = this; // Используем временную переменную для текущего объекта
        ...
        for (char c : str.toCharArray()) {
            if (c == '{') {
                ...
                Multiset nestedMultiset = new Multiset();
                isNestedMultiset = true;
                currentMultiset.addNestedMultiset(nestedMultiset);
                currentMultiset = nestedMultiset; // Обновляем текущий объект
            } else if (c == '}') {
                if (currentElement.length() > 0) {
                    currentMultiset.addElement(currentElement.toString());
                    currentElement.setLength(0);
                }
                currentMultiset = stack.pop(); // Обновляем текущий объект
                isNestedMultiset = false;
            } else if (c == ',') {
                if (currentElement.length() > 0) {
                    currentMultiset.addElement(currentElement.toString());
                    currentElement.setLength(0);
    ...
    public void createMultisetFromArray(char[] array) {
        StringBuilder currentElement = new StringBuilder();
        boolean isNestedMultiset = false;
        Stack<Multiset> stack = new Stack<>();
        ...
        for (char c : array) {
            if (c == '{') {
                ...
                Multiset nestedMultiset = new Multiset();
                currentMultiset.addNestedMultiset(nestedMultiset);
                currentMultiset = nestedMultiset;
                isNestedMultiset = true;
            } else if (c == '}') {
                if (currentElement.length() > 0) {
                    currentMultiset.addElement(currentElement.toString());
